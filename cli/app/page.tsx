...
export default function Home() {
  ...
  async function LoginNormal() {
    ...
    if (res.message === "Error" && res.error) {
      ...
      // Erfolgreich eingeloggt (optional: Weiterleitung oder Success-Meldung)
  ...
  return (
    <div className="min-h-screen flex flex-col justify-start items-center px-6 pt-10 space-y-6">
      <div className="w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl">
        ...
        <div className="w-full space-y-4 mt-6">
          <Input placeholder="Email" className="w-full" type="email" autoComplete="email webauthn" error={emailError}
            onChange={(e) => setEmail(e.target.value)}
            value={email}
          />
          <Input placeholder="Password" type="password" autoComplete="new-password webauthn" className="w-full" error={passwordError}
